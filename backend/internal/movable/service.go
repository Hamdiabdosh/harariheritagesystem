...
func (s *Service) GetByID(ctx context.Context, id, userID uuid.UUID, role models.Role) (*RecordDetail, error) {
	...
	return &RecordDetail{
		...
		Comments: listComments(ctx, s.commentLister, models.RecordTypeMovable, id),
...
func validateForSubmit(record *models.MovableRecord) error {
	...
	if record.Woreda == nil || strings.TrimSpace(*record.Woreda) == "" {
		fields["woreda"] = "required"
	}
	if record.Kebele == nil || strings.TrimSpace(*record.Kebele) == "" {
